...
contract StakeRewards is IStakeRewards, Ownable {
  address public defi;
  IERC721 private sDEFIAddress;
  IERC721 private slDEFIAddress;
  ...
  modifier onlyToken() {
    require(
      msg.sender == address(sDEFIAddress) || msg.sender == address(slDEFIAddress),
      'must be stake token'
    );
    _;
  }
  ...
  function _distributeReward(address shareholder, bool compound) internal {
    ...
    if (amount > 0) {
      ...
      if (compound) {
        ...
        bool success;
        (success, ) = payable(shareholder).call{ value: amount }('');
      }
  ...
  function getsDEFI() external view returns (address) {
    return address(sDEFIAddress);
  }

  function getslDEFI() external view returns (address) {
    return address(slDEFIAddress);
  }
  ...
  function setsDEFI(address _sDEFI) external onlyOwner {
    require(_sDEFI != address(0), "Zero address");
    sDEFIAddress = IERC721(_sDEFI);
  }

  function setslDEFI(address _slDEFI) external onlyOwner {
     require(_slDEFI != address(0), "Zero address");
    slDEFIAddress = IERC721(_slDEFI);
  }
}

contract DEFIVester is Ownable {
  IDEFI private _defi;
  ...
  function withdraw(uint256 _index) external {
    ...
    _vest.withdrawsCompleted = _withdrawsAllowed;
    _defi.transfer(_user, _tokensPerWithdrawPeriod * _withdrawsToComplete);
    _defi.addToBuyTracker(
      _user,
  ...
  function getDEFI() external view returns (address) {
    return address(_defi);
  }
...
contract slDEFI is DEFIStake {
  ...
  function zapAndStakeETHOnly(uint256 _lockOptIndex) external payable {
    ...
    uint256 _defiBalanceBefore = defi.balanceOf(address(this));
    IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(_uniswapRouter);

    // swap half the ETH for DEFI
    uint256 _tokensToReceiveNoSlip = _getTokensToReceiveOnBuyNoSlippage(
      msg.value / 2
    );
    address[] memory path = new address[](2);
    ...
    _addLp(defi.balanceOf(address(this)) - _defiBalanceBefore, msg.value / 2);
    ...
    _returnExcessDEFI(msg.sender, _defiBalanceBefore);

    emit ZapETHOnly(msg.sender, msg.value, _lpBalanceToStake);
  }

  function zapAndStakeETHAndDEFI(uint256 _amountDEFI, uint256 _lockOptIndex)
    external
    payable
  {
    require(msg.value > 0, 'need to provide ETH to zap');
    ...
    uint256 _ethBalBefore = address(this).balance - msg.value;
    uint256 _defiBalBefore = defi.balanceOf(address(this));
    defi.transferFrom(msg.sender, address(this), _amountDEFI);
    uint256 _defiToProcess = defi.balanceOf(address(this)) - _defiBalBefore;

    uint256 _lpBalBefore = stakeToken.balanceOf(address(this));
    _addLp(_defiToProcess, msg.value);
    uint256 _lpBalanceToStake = stakeToken.balanceOf(address(this)) -
      _lpBalBefore;
    _stakeLp(msg.sender, _lpBalanceToStake, _lockOptIndex, false);

    _returnExcessETH(msg.sender, _ethBalBefore);
    _returnExcessDEFI(msg.sender, _defiBalBefore);

    emit ZapETHAndDEFI(msg.sender, msg.value, _amountDEFI, _lpBalanceToStake);
  }

  function zapAndStakeDEFIOnly(uint256 _amountDEFI, uint256 _lockOptIndex)
    external
  {
    require(
      _aprLockOptions[_lockOptIndex].lockTime > 0,
      'cannot zap and stake DEFI only without lockup period'
    );
    ...
    defi.approve(address(_uniswapV2Router), _defiToProcess / 2);
    ...
    uint256 _lpBalBefore = stakeToken.balanceOf(address(this));
    _addLp(_defiToProcess / 2, address(this).balance - _ethBalBefore);
    uint256 _lpBalanceToStake = stakeToken.balanceOf(address(this)) -
      _lpBalBefore;
    _stakeLp(msg.sender, _lpBalanceToStake, _lockOptIndex, false);
    ...
    emit ZapDEFIOnly(msg.sender, _amountDEFI, _lpBalanceToStake);
  }

  function _addLp(uint256 tokenAmount, uint256 ethAmount) private {
    IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(_uniswapRouter);
    defi.approve(address(_uniswapV2Router), tokenAmount);
  ...
  function _getETHToReceiveOnSellNoSlippage(uint256 _amountDEFI)
    internal
    view
    returns (uint256)
  {
    ...
    (uint112 _r0, uint112 _r1, ) = pair.getReserves();
    if (pair.token0() == IUniswapV2Router02(_uniswapRouter).WETH()) {
      return (_amountDEFI * _r0) / _r1;
    } else {
      return (_amountDEFI * _r1) / _r0;
    }
  }
  ...
  function _stakeLp(
    ...
    uint256 defiReserves = t0 == address(defi) ? res0 : res1;
    uint256 singleSideTokenAmount = (_amountStakeToken * defiReserves) /
  ...
  function _returnExcessETH(address _user, uint256 _initialBal) internal {
    if (address(this).balance > _initialBal) {
      (bool success, ) = payable(_user).call{ value: address(this).balance - _initialBal }('');
      require(success, "Eth Send Failed");
      require(address(this).balance >= _initialBal, 'took too much');
    }
  }

  function _returnExcessDEFI(address _user, uint256 _initialBal) internal {
    uint256 _currentBal = defi.balanceOf(address(this));
    if (_currentBal > _initialBal) {
      defi.transfer(_user, _currentBal - _initialBal);
      require(defi.balanceOf(address(this)) >= _initialBal, 'took too much');
    }
  }
...
contract DEFI is IDEFI, ERC20, Ownable {
  ...
  sDEFI private _stakedDEFI;
  slDEFI private _stakedDEFILP;
  DEFIVester private _vester;
  ...
  modifier onlyStake() {
    require(
      address(_stakedDEFI) == _msgSender() ||
        address(_stakedDEFILP) == _msgSender(),
  ...
  constructor() ERC20('Degenerate Finance', 'DEFI') {
    ...
    _vester = new DEFIVester(address(this));
    _rewards = new StakeRewards(address(this), address(_uniswapV2Router));

    _stakedDEFI = new sDEFI(
      address(this),
      address(_vester),
      address(_rewards),
      ''
    );
    _stakedDEFI.setPaymentAddress(msg.sender);
    _stakedDEFI.setRoyaltyAddress(msg.sender);
    _stakedDEFI.transferOwnership(msg.sender);

    _stakedDEFILP = new slDEFI(
      uniswapV2Pair,
    ...
    _stakedDEFILP.setPaymentAddress(msg.sender);
    _stakedDEFILP.setRoyaltyAddress(msg.sender);
    _stakedDEFILP.transferOwnership(msg.sender);

    _vester.addStakingContract(address(_stakedDEFI));
    _vester.addStakingContract(address(_stakedDEFILP));
    _vester.transferOwnership(msg.sender);

    _rewards.setsDEFI(address(_stakedDEFI));
    _rewards.setslDEFI(address(_stakedDEFILP));
    ...
    isTaxExcluded[address(_stakedDEFILP)] = true; // allow zapping without taxes
  ...
  function _transfer(
    ...
    // handles transferring to a fresh wallet or wallet that hasn't bought DEFI before
  ...
  function _swapForETH(uint256 _amountToSwap) private swapLock {
    ...
    if (_balToProcess > 0) {
      ...
      if (_marketingETH > 0) {
        (bool success1,) = payable(_treasury).call{ value: _marketingETH }('');
        require(success1, "Eth send failed");
      }
      ...
      if (_rewardsETH > 0) {
        if (_rewards.totalSharesDeposited() > 0) {
          ...
          (bool success2, ) =  payable(_treasury).call{ value: _rewardsETH }('');
          require(success2, "Eth send failed");
        }
      }
    }
  }
  ...
  function getStakedDEFI() external view returns (address) {
    return address(_stakedDEFI);
  }

  function getStakedDEFILP() external view returns (address) {
    return address(_stakedDEFILP);
  }
